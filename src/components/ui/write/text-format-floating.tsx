...
import type { blockTypeToBlockName } from "@/components/providers/ToolbarContext"
import {
  $getSelection,
...
export default function TextFormatFloatingToolbar({
  ...
  useEffect(() => {
    ...
    // Listen for focus/blur events that might indicate keyboard open/close
    const handleFocusIn = () => handleKeyboardToggle()
    const handleFocusOut = () => handleKeyboardToggle()

    // Listen for resize events that might indicate keyboard open/close on mobile
    const handleResize = () => {
  ...
  return (
    ...
    >
      ...
      <Separator orientation="vertical" className="max-sm:h-[1px] max-sm:my-1 max-sm:w-5 h-5 mx-1" />
      <BlockFormatDropDown
        editor={editor}
        blockType={blockType}
        style={{ height: "25px" }}
